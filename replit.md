...
-   **API Documentation Explorer**: Searchable, filterable catalog of all 270 API endpoints across 18 categories with HTTP method badges, authentication requirements, request/response examples, query parameters, and sample data. Accessible at /api-explorer (no login required).
...
-   **Statistics Verification System**: Automated verification script (`verify-stats.ts`) dynamically validates platform metrics (105 features, 144 tables, 270 endpoints, 95 services) across Demo, API Explorer, and documentation to ensure accuracy. Includes pre-commit hook template.
...
-   **Living Policy Manual**: Ebook-style browser transforming 25 golden policy sources into organized chapters with page numbering (500 words/page), legal citations, glossary extraction, and direct links to Rules as Code implementations. Features 3-column interface with chapter/section tree navigation, full-text search, and responsive design. Includes 4 database tables and 6 API routes.
-   **Dynamic Notification Engine**: Auto-generates official notices (SNAP approval/denial, tax reminders) pulling real-time data from Rules as Code, eliminating static templates and version control issues. Features content rules mapping variables to RaC sources, full audit trail with RaC version tracking, multi-channel delivery, and policy-driven regeneration when rules change. Includes 4 database tables and 5 API routes.
-   **Shared Glossary API Service**: Centralized glossary terms across all 6 programs with autocomplete search (GET /api/policy-manual/glossary/autocomplete) and tooltip integration (GlossaryTooltip.tsx component). Provides consistent definitions with legal citations, usage examples, and cross-program term relationships. Includes 1 database table (policy_glossary_terms) and 4 API routes for CRUD operations.
-   **Modular Form Builder**: Drag-and-drop UI for assembling official notices and documents from reusable components using @dnd-kit/sortable. Enables non-technical content editors to create policy-driven templates. Features component library with headers, footers, calculations, and signatures. Includes FormBuilderPage.tsx with visual editor, 1 database table (form_components), and 6 API routes for component management.
-   **Rules-to-Content Pipeline**: Automated detection of Rules as Code changes with content sync job generation. Runs hourly cron job (rulesContentSyncService.ts) monitoring 12 RaC tables (snap_income_limits, medicaid_fpl, etc.) to identify content requiring updates. Features content_rules_mapping table linking RaC fields to affected templates, configurable auto-regeneration policies, and complete audit trail. Includes 2 database tables (content_rules_mapping, content_sync_jobs) and 9 API routes.
-   **Admin Content Dashboard**: Comprehensive 4-tab admin interface (AdminContentDashboard.tsx) for reviewing sync jobs, managing templates, viewing analytics, and configuring automation settings. Sync Jobs tab shows pending RaC change reviews with approve/reject workflow. Templates tab displays searchable card grid with usage statistics. Analytics tab shows 10+ metrics with Recharts visualizations (bar, pie, line charts). Settings tab controls auto-regeneration policies and notification channels. Includes 1 database table (content_sync_settings) and 3 API routes for analytics and settings.
-   **Advanced Policy Search**: Enhanced search with 3 modes (keyword, citation, semantic) and filters by program, date range, and Rules as Code availability (AdvancedSearchPanel.tsx). Citation mode searches specifically in legalCitation and sourceUrl fields. Semantic mode includes synonym expansion (income→earnings/wages/salary, benefits→assistance/aid). Integrated into PolicyManualBrowser.tsx as slide-in Sheet panel. Includes 3 API routes (search, citations, filters) with support for multi-program filtering and date range queries.
-   **Version Comparison UI**: Side-by-side diff view for policy evolution tracking with version history timeline (VersionHistoryTimeline.tsx), synchronized scrolling, and color-coded change highlighting (VersionCompareDialog.tsx). Tracks changes to manual sections and notification templates with complete audit trail. Features line-by-line diff algorithm (diffGenerator.ts) with word-level highlighting. Includes 2 database tables (policy_manual_versions, template_versions) and 5 API routes for version history, diff generation, and comparison.
