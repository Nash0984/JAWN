/**
 * WCAG 2.1 AAA Accessibility Audit using Puppeteer + axe-core
 * Scans all 30 priority pages for accessibility violations
 */
...
async function login(page: Page, role: 'client' | 'navigator' | 'admin') {
  ...
  await page.type('[data-testid="input-username"]', AUTH_CREDENTIALS[role].username);
  await page.type('[data-testid="input-password"]', AUTH_CREDENTIALS[role].password);
...
async function scanPage(
  ...
  try {
    ...
    await new Promise(resolve => setTimeout(resolve, 1500)); // Give dynamic content time to render
    ...
    const totalElements = violations.reduce((sum: number, v: any) => sum + v.nodes.length, 0);
    
    console.log(`  ✓ Found ${violations.length} violation types affecting ${totalElements} elements`);
...
function categorizeImpactArea(violationId: string): string {
  ...
  if (violationId.includes('label') || violationId.includes('input') || violationId.includes('form')) return 'Forms';
  if (violationId.includes('image') || violationId.includes('img') || violationId.includes('alt')) return 'Images';
  if (violationId.includes('heading') || violationId.includes('landmark') || violationId.includes('region')) return 'Screen readers';
  return 'Other';
}
...
- **WCAG Criteria:** ${violation.tags.filter((t: string) => t.startsWith('wcag')).join(', ')}
...
**Focus Areas:**
${Object.entries(summary.violationsByImpactArea).sort(([,a], [,b]) => b - a).slice(0, 3).map(([area, count]) => `- ${area} (${count} violations)`).join('\n')}
`;
...
async function main() {
  ...
  try {
    ...
    const chromiumPath = process.env.CHROMIUM_PATH || '/nix/store/.../chromium-...';
    const executablePath = process.env.PUPPETEER_EXECUTABLE_PATH || 
                          (await puppeteer.executablePath().catch(() => 'chromium'));
    
    browser = await puppeteer.launch({
      headless: true,
      executablePath: executablePath === 'chromium' ? 'chromium' : executablePath,
      args: [
        '--no-sandbox',
